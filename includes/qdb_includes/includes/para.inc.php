...
if (!preg_match("/\bupgrade.php\b/i", "$tmp") && !preg_match("/\bquotes.php\b/i", "$tmp")) { error("Hacking Attempt"); }
...
if($boardClosedCheck==1 && !isset($_GET['admin'])){
 ...
 $sql2 = $sql->fetchArray();
  ...
  else if (isset($_GET['q']))   { $quotenum=$_GET['q']; view($quotenum); }
