...
global $lang;
  echo("\n\n<div class='sec'><h1><a href='?q=$id' title='$lang[output_comment_2]'><b>#$id</b></a></h1><h2></h2>\n");

  echo(" ($vote) <a href='?sux=$id'>[X]</a><font size=3><br /><code>$quote</code><br />\n");
...
$quote = escape_str($quote);
  ...
  if(!is_numeric($quote)){ problem(); } 
    ...
    if($sql2["quote"]==""){
      echo("<div align=center><font size='-1'>$lang[error_comment_3]<br /><br />$lang[back_link_1]</font></div>");
    } else if($sql2["approved"]!="1"){
      echo("<div align=center><font size='-1'>$lang[vote_comment_1]</font></div>");
      $id=$sql2["id"];
      $quote=$sql2["quote"];
      $vote=$sql2["score"];
      $comment=$sql2["comment"];
